...
**Tests (228 total):** `planFieldChanges_test.php` (planner + delimiter regression, 36), `recordLogging_test.php` (capture / `buildHumanChanges` / per-record logging via global `\REDCap` stub, 28), `processProject_test.php` (transaction + post-commit isolation, 16), `elementEnum_test.php` (parse/serialize/round-trip corruption guard, byte-exact fixtures, 21), `cleanup_test.php` (dup consolidation, ack gating, idempotency, vmap backward-compat, isolation, 31), `repeatingInstances_test.php` (repeating-instrument read flatten + save-row split, 28 — see `REPEATING_INSTRUMENTS_FIX.md`), `syncMatch_test.php` (empty-value/array sync matching, accept-OnCore-data write values, study-site push error, 39 — see `EMPTY_VALUES_SYNC_FIX.md`), `pushBooleans_test.php` (OnCore boolean demographics conversion, value_mapping override, named validation, 29 — see `PUSH_BOOLEANS_FIX.md`).

### Empty-value sync matching (`determineSyncedRecordMatch`)

OnCore array fields without a value mapping (e.g. `additionalSubjectIds`) arrive as `[]` (fresh API pull) or the JSON string `"[]"` (`redcap_entity_oncore_subjects` cache — the common case). `Subjects::flattenOnCoreValue()` canonicalizes both to `''` (non-empty arrays join to `'A, B'`); `determineSyncedRecordMatch` flattens both sides of unmapped text compares and treats both-sides-empty as a match for value-mapped fields (an empty OnCore value has no vmap entry but there is nothing to map). `prepareOnCoreRecordForSync` (Accept OnCore Data) saves flattened values — never raw JSON junk like the literal string `[]` — and leaves the REDCap field untouched when a non-empty OnCore value has no vmap entry (instead of blanking it). The array-mapped radio/text compare must iterate `$parsed` (decoded), NOT the raw value — cached subjects deliver JSON strings and `foreach` over a string silently skips the compare. `getOnCoreStudySite` throws an informative error (site label + configured sites) when a record's study site is not in the push vmap; only a genuinely empty site falls through to the generic "Study site is missing". Full background in `EMPTY_VALUES_SYNC_FIX.md`.

### Boolean fields on push (`approximateBirthDate`, `birthDateNotAvailable`, `approximateExpiredDate`)

OnCore rejects any non-JSON-boolean value for these three fields with `"Invalid boolean. Valid values are [true, false]"` and `field: null` (no field name) — confirmed both that sending REDCap's raw `'0'`/`'1'` strings always fails (regardless of value) and that omitting the keys entirely always succeeds (no interdependency with `birthDate`/`expiredDate` or other fields, unlike e.g. a partially-filled address). `Subjects::toOnCoreBoolean()` converts REDCap values (`'1'/'0'/'true'/'false'/'yes'/'no'`, case-insensitive, plus real bool/int) to a real `bool`, or `null` to omit the key. `prepareREDCapRecordForSync` and `fillMissingData` check `isOnCoreBooleanType()` **before** checking `value_mapping` — a value_mapping on these fields is reachable via the Field Mapping UI (it renders a value-mapping table whenever the *REDCap* field has enumerated choices, without checking whether the *OnCore* field is boolean and therefore has no `oncore_valid_values` to offer) but is always wrong to honor, since it stores a string. `createOnCoreProtocolSubject` also validates these fields right before the API call and names the offending field in the error (OnCore's own response never does). Full background in `PUSH_BOOLEANS_FIX.md`.
