...
**Tests (199 total):** `planFieldChanges_test.php` (planner + delimiter regression, 36), `recordLogging_test.php` (capture / `buildHumanChanges` / per-record logging via global `\REDCap` stub, 28), `processProject_test.php` (transaction + post-commit isolation, 16), `elementEnum_test.php` (parse/serialize/round-trip corruption guard, byte-exact fixtures, 21), `cleanup_test.php` (dup consolidation, ack gating, idempotency, vmap backward-compat, isolation, 31), `repeatingInstances_test.php` (repeating-instrument read flatten + save-row split, 28 — see `REPEATING_INSTRUMENTS_FIX.md`), `syncMatch_test.php` (empty-value/array sync matching, accept-OnCore-data write values, study-site push error, 39 — see `EMPTY_VALUES_SYNC_FIX.md`).
